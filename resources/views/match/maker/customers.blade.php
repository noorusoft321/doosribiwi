...
@push('script')
    ...
    <script>
        ...
        let listTable = '';
        let currentUserRoleId = '{{($currentUserRoleId==8) ? false : true}}';
            ...
            listTable = $('.viewTableData').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        data: 'faker_id',
                        render: function (data, type, full, meta) {
                            return full.match_assign_lead_user_name;
                        }
                    },
                    {
                        data: 'full_name'
                    ...
                    },
                    {
                        ...
                        render: function (data, type, full, meta) {
                            return (
                                `
                                <a target="_blank" class="fs-3" href="{{route('match.get.customer.center.detail')}}/${data}" title="View Detail"><i class="fa fa-eye"></i></a>
                            `
                            );
                        }
                    }
            ...
            listTable = $('.viewTableData').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        data: 'faker_id',
                        render: function (data, type, full, meta) {
                            return full.match_assign_user_name;
                        }
                    },
                    {
                        data: 'full_name'
        ...
        function assignUser(input) {
            ...
            if (assignUserId=='') {
                alertyFy('Please select assign user*','warning',3000);
                return false;
            }
            if(confirm('Are you sure you want to assign?')){
                $(input).attr('disabled',false);
                axios.post("{{route('match.customers.assign')}}", `${$('#assignUserForm').serialize()}&assign_user_id=${assignUserId}`).then(function (res) {
