...
@section('content')
        ...
        <div class="page-content">
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                ...
                <div class="ms-auto">
                    ...
                    @endif
                    <button onclick="showRequestModal()" class="btn btn-success">Request</button>
                </div>
            </div>
                    ...
                    @if($currentUserRoleId != 4)
                        <div class="filter mb-3">
                            ...
                            <div class="row">
                                ...
                                <div class="col-3 pb-2">
                                    <div class="form-group">
                                        <label for="f_type">Type (Source)</label>
                                        <select name="f_type" id="f_type" class="form-control">
                                            <option value=""> -- All Customers -- </option>
                                            <option value="web">Website Registration</option>
    ...
    <div class="modal fade" id="requestModal" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">New Leads Request</h4>
                    <button type="button" class="close closeModal">&times;</button>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                ...
                <div class="modal-footer">
                    <button onclick="requestUser(this)" type="button" class="btn btn-primary">Request</button>
                    <button type="button" class="btn btn-danger closeModal">Close</button>
                </div>
            </div>
        </div>
...
@push('script')
    ...
    <script>
        ...
        let listTable = '';
        let currentUserRoleId = '{{($currentUserRoleId==3) ? false : true}}';

        if (currentUserRoleId==true) {
            listTable = $('.viewTableData').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        data: 'faker_id',
                        render: function (data, type, full, meta) {
                            return full.assign_lead_user_name;
                        }
                    },
                    {
                        data: 'full_name'
                    ...
                    },
                    {
                        ...
                        render: function (data, type, full, meta) {
                            return (
                                `
                                <a target="_blank" class="fs-3" target="_blank" href="{{route('bdo.get.customer.center.detail')}}/${data}" title="View Detail"><i class="fa fa-eye"></i></a>
                            `
                            );
                        }
                    }
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, full, meta) {
                            return (
                                `
                                <a target="_blank" class="fs-3" href="{{route('bdo.get.customer.center.detail')}}/${data}" title="View Detail"><i class="fa fa-eye"></i></a>
                            `
                            );
                        }
                    }
        ...
        function requestUser(input) {
            let requestNumber = $('select[name="request_number"]').val();
            if (requestNumber==0) {
                alertyFy('Please select request number*','warning',3000);
                return false;
            }
            if(confirm('Are you sure you want to request new leads?')){
                $(input).attr('disabled',false);
                axios.post("{{route('bdo.customers.request')}}", {
                    ...
                    if (res.data.status=='success') {
                        ...
                        alertyFy(res.data.msg,res.data.status,0);
                    }
                    $(input).attr('disabled',false);
                }).catch(function (error) {
                    alertyFy('There is something wrong','warning',3000);
        ...
        function showRequestModal() {
            $('#requestModal select[name="request_number]"]').val('0');
