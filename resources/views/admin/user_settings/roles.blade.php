                ...
                <form id="addEditForm">
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" id="title" class="form-control" placeholder="Role Name">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        <button onclick="saveRole(this);" type="button" class="btn btn-primary">Save changes</button>
                    </div>
                </form>
            </div>
        </div>
...
@push('script')
    ...
    <script>
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        return (
                            `
                                <a class="fs-3" onclick="addEditModal(this,'${data}','${full.title}')" href="javascript:void(0)"><i class="fa fa-edit"></i></a>
        ...
        function addEditModal(input, mainId='',title='') {
            $(':input').removeClass('has-error');
            $('.text-danger').remove();
            if (mainId) {
                $('#addEditModal #addEditModalLabel').text('Edit');
                createUpdateUrl = '{{route('admin.role.save')}}/' + mainId;
                $('#addEditModal input[name="title"]').val(title);
            } else {
                $('#addEditModal #addEditModalLabel').text('Add');
                createUpdateUrl = '{{route('admin.role.save')}}';
                $('#addEditModal input[name="title"]').val('');
            }
        ...
        function deleteRecord(input, mainId) {
            ...
            if (confirm("Are you sure you want to delete this?")) {
                $.get(`{{route('admin.role.delete')}}/${mainId}`, function (res) {
