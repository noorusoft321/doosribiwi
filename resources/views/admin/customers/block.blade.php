			...
			<div class="card radius-10">
				<div class="card-body">
					<div class="d-flex align-items-center">
						<div>
							<h5 class="mb-0"> Block Customers List </h5>
						</div>
					</div>
					<form>
						<div class="row">
							...
							<div class="col-auto text-end">
								...
								<a href="{{route('admin.get.all.block.customers')}}" type="button" class="btn btn-success">Reset</a>
							</div>
						</div>
					</form>
					<hr>
					<div class="table-responsive">
						<table class="table table-hover align-middle mb-0">
							...
							<tbody>
								@foreach($blockCustomers as $customer)
									<tr>
										<td>{{$customer->id}}</td>
										<td>
											<a href="{{route('admin.get.customer.detail',[$customer->faker_block_by])}}">{{$customer->block_by_name}}</a>
										</td>
										<td>
											<a href="{{route('admin.get.customer.detail',[$customer->faker_block_to])}}">{{$customer->block_to_name}}</a>
										</td>
										<td>{{$customer->desc}}</td>
					...
					@if ($blockCustomers->hasPages())
						<div class="pagination-wrapper">
							{{ $blockCustomers->links() }}
						</div>
					@endif
